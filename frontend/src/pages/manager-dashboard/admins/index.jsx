// frontend\src\pages\manager-dashboard\admins\index.jsx
"use client";
...
import {
  ...
  Calendar,
  Zap,
} from "lucide-react"; // استفاده از لوسید برای هماهنگی با سبک
import Link from "next/link";
import DashboardLayout from "../layout";
import Swal from "sweetalert2";

// کارت کاربران برای موبایل با استایل باشگاه
const UserCard = ({ user, index, handleDelete }) => {
  const isActive = user.status === "ACTIVE";

  return (
    <div className="bg-[#1a1d23] p-5 rounded-2xl border border-gray-800 hover:border-blue-400/50 transition-all duration-300 shadow-xl group">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            {user.profileImage ? (
              <img
                ...
                alt={user.name}
                className="w-14 h-14 object-cover rounded-xl border-2 border-blue-400"
              />
            ) : (
              <div className="w-14 h-14 flex items-center justify-center bg-gray-800 text-blue-400 rounded-xl border-2 border-gray-700 text-xl font-black">
                {user.name.charAt(0)}
              </div>
            )}
            <div
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 uppercase tracking-wider">
              ID: {user.employeeCode} | {user.role}
            </p>
          </div>
        </div>
        <span
          className={`px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tighter ${
            isActive ? "bg-blue-400 text-black" : "bg-gray-700 text-gray-300"
          }`}
        >
      ...
      <div className="space-y-3 text-[13px] text-gray-400">
        <div className="flex items-center gap-2">
          ...
          <span>{user.phone || "بدون شماره"}</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="flex justify-end gap-2 mt-5">
        <Link
          href={`/manager-dashboard/admins/${user.id}/edit`}
...
export default function UsersPage() {
  ...
  const users = data?.users ? data.users.filter((u) => u.role === "Admin") : [];
      ...
      <div className="p-4 sm:p-8 min-h-screen bg-[#0F1420] rounded-[2.5rem] border border-gray-800 shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div className="flex items-center gap-4">
            <Link
              ...
              className="p-3 bg-gray-800 text-blue-400 hover:bg-blue-400 hover:text-black transition-all rounded-xl shadow-lg"
            >
              <ArrowRight size={24} />
            </Link>
            <div>
              <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase">
                مدیریت <span className="text-blue-400">مدیران</span>
              </h2>
          ...
          <Link
            href="/manager-dashboard/admins/create"
            className="w-full md:w-auto flex items-center justify-center gap-2 bg-blue-400 hover:bg-blue-500 text-black px-6 py-4 rounded-2xl text-sm font-black transition-all shadow-[0_10px_20px_rgba(250,204,21,0.2)] active:scale-95"
          >
        ...
        <div className="mb-8 relative group">
          <input
            ...
            className="w-full bg-[#1a1d23] border border-gray-800 text-white rounded-2xl p-4 pr-12 focus:outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-400/10 transition-all shadow-inner"
          />
          <Search
          ...
          <table className="w-full text-right">
            <thead>
              <tr className="bg-gray-800/50 text-gray-400 text-[11px] uppercase tracking-[0.2em] border-b border-gray-700">
                ...
                <th className="py-5 px-6 font-black">اساتید</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {filteredUsers.length === 0 ? (
                <tr>
                  ...
                  >
                    ⚠️ هیچ اساتیدی در این لیست یافت نشد
                  </td>
                </tr>
              ) : (
                filteredUsers.map((user, index) => (
                  <tr
                    key={user.id}
                    className="hover:bg-blue-400/5 transition-colors group"
                  >
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        {user.profileImage ? (
                          ...
                          <div className="w-10 h-10 bg-gray-800 rounded-lg flex items-center justify-center text-blue-400 font-bold text-xs border border-gray-700">
                            {user.name.charAt(0)}
                          </div>
                        )}
                    ...
                    <td className="py-4 px-6 text-center">
                      <span
                        className={`inline-block w-2.5 h-2.5 rounded-full ${
                          user.status === "ACTIVE"
                            ? "bg-blue-400 shadow-[0_0_10px_rgba(250,204,21,0.5)]"
                            : "bg-gray-600"
                        }`}
                      ></span>
                    </td>
